...
export function QuotePDF({ org, project }: QuotePDFProps) {
  ...
  const topLevelItems = project.lineItems.filter((i) => !i.isKitChild && !i.isAccessory);
            ...
            {Array.from(groups.entries()).map(([groupName, items]) => (
              <View key={groupName}>
                ...
                {items.map((item, idx) => {
                  ...
                  // For KIT_PRICE kits, only show accessories (no kit children pricing). Otherwise show all children.
